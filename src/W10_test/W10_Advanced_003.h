...
char mqtt_server[40] = "";        // MQTT 서버 주소
char mqtt_port[6]    = "8080";    // MQTT 포트
char api_token[34]   = "YOUR_APITOKEN"; // API 토큰
char static_ip[16]   = "10.0.1.56"; // 고정 IP 주소
char static_gw[16]   = "10.0.1.1";  // 고정 게이트웨이 주소
char static_sn[16]   = "255.255.255.0"; // 고정 서브넷 마스크
    ...
    if (LittleFS.begin()) {
        ...
        if (LittleFS.exists(G_W10_WM_CONFIG_FILE)) {
            ...
            if (v_configFile) {
                ...
                buf.get()[size] = '\0'; // 널 종료 문자 추가
                v_configFile.close();
 
                JsonDocument json;
                // JSON 파싱 시도
                auto deserializeError = deserializeJson(json, buf.get());
                serializeJson(json, Serial); // 디버깅용: 파싱된 JSON 내용을 시리얼 모니터에 출력
                Serial.println(); // 개행
                ...
                if ( ! deserializeError ) {
                    ...
                    // 논블로킹 모드 설정 로드
                    g_W10_wm_nonblocking = json["wm_nonblocking"] | false;
...
void W10_saveJson_config(){
    if (g_W10_shouldSaveConfig) {
        ...
        serializeJson(json, Serial); // 디버깅용: 저장될 JSON 내용을 시리얼 모니터에 출력
        Serial.println(); // 개행
        serializeJson(json, configFile); // 파일에 JSON 내용 쓰기
...
void W10_init() {
    ...
    W10_loadJson_config(); // 저장된 JSON 설정 파일 로드
    ...
    char checkbox_checked[10]; // "checked" 또는 "" 저장
    if (g_W10_wm_nonblocking) {
        strcpy(checkbox_checked, "checked");
    } else {
        strcpy(checkbox_checked, "");
    }
    // HTML 형태의 체크박스 매개변수
    ...
    new (&g_W10_custom_field) WiFiManagerParameter(custom_radio_str);
    ...
    res = g_W10_WifiManager.autoConnect("AutoConnectAP", "password");
    ...
    W10_saveJson_config(); // 모든 변경된 설정들을 JSON 파일에 저장
